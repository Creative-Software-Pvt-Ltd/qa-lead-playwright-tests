...
test.describe('Inventory flow', () => {
  ...
  // placing an order through UI, fetch it in API and validate order details
  test(
    ...
    async ({ page, request }) => {
      ...
      // Validate item 1
      expect(orderBody.order.items[0].productId).toBe(orderItems[0].productId);

      expect(orderBody.order.items[0].quantity).toBe(orderItems[0].quantity);

      expect(orderBody.order.items[0].name).toBe(orderItems[0].name);

      expect(orderBody.order.items[0].lineTotal).toBe(orderItems[0].price);

      // Validate item 2
      expect(orderBody.order.items[1].productId).toBe(orderItems[1].productId);

      expect(orderBody.order.items[1].quantity).toBe(orderItems[1].quantity);

      expect(orderBody.order.items[1].name).toBe(orderItems[1].name);

      expect(orderBody.order.items[1].lineTotal).toBe(orderItems[1].price);

      // Validate total
      const calculatedTotal = orderBody.order.items[0].lineTotal + orderBody.order.items[1].lineTotal;

      expect(orderBody.order.total).toBe(calculatedTotal);
