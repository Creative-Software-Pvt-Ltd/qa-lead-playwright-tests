...
test('validate the cart', async ({page}) => { 
    ...
    const wirelessHeadphonesQty = Number(await pages.cartPage.getCartItemQuantity(orderItems[0].productId).textContent());
    const smartWatchQty = Number(await pages.cartPage.getCartItemQuantity(orderItems[1].productId).textContent());

    expect(wirelessHeadphonesQty).toBe(orderItems[0].quantity);
    expect(smartWatchQty).toBe(orderItems[1].quantity);

    const subtotalText = await pages.cartPage.subtotalAmount.textContent();
...
test('Log the Order ID', async () => {
    ...
    const orderId = await pages.checkoutPage.orderIdText.textContent();
